...
int main(int argc, char *argv[])
{
	...
	Mat src = imread("../data/DIP3E_Original_Images_CH08/Fig0809(a).tif", 0);
	Mat logo = imread("../data/DIP3E_Original_Images_CH08/Fig0850(a).tif", 0);
	...
	for (int i = 0; i < src_to_jpg.rows; i++)
	{
		...
		{
			logo_built2.data[i*src_to_jpg.cols + j] = src_to_jpg.data[i*src_to_jpg.cols + j] & 0X3;
		}
	}
