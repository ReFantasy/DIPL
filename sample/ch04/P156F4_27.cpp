...
{
	// 妇女原图
	std::string image_path = "../data/DIP3E_Original_Images_CH04/Fig0427(a)(woman).tif";
	...
	// fig 4.24谱和妇女的相位角
	image_path = "../data/DIP3E_Original_Images_CH04/Fig0424(a)(rectangle).tif";
	...
	for (int i = 0; i < rows; i++)
		for (int j = 0; j < cols; j++)
		{
			_e[i][j] = { abs(_fig4_24[i][j])*cos(angle[i][j]),abs(_fig4_24[i][j])*sin(angle[i][j]) };
		}
	_e = fourier.IFFT(_e);
