...
using namespace std;
Mat ILPFExampel(const Mat &src, double D0)
{
    ...
    // 生成滤波核
    auto kernel_fft = IPL::ILPF(src_fft.rows, src_fft.cols, D0);
...
int main(int argc, char *argv[])
{
    ...
    auto b = ILPFExampel(src, 10);
    auto c = ILPFExampel(src, 30);
    auto d = ILPFExampel(src, 60);
    auto e = ILPFExampel(src, 160);
    auto f = ILPFExampel(src, 460);
