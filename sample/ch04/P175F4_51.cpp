...
{
    std::string image_path = std::string(DATA_DIR) +
                             "/DIP3E_Original_Images_CH04/Fig0451(a)(satellite_original).tif";
    Mat src = imread(image_path, 0);

    auto b = GLPFExampel(src, 50);
    auto c = GLPFExampel(src, 20);
