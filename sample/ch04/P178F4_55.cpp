...
using namespace std;
Mat BHPFExampel(const Mat &src, double D0)
{
	...
	// 生成滤波核
	auto kernel_fft = IPL::BHPF(src_fft.rows, src_fft.cols, D0);
...
int main(int argc, char *argv[])
{
	...
	auto a = BHPFExampel(src, 30);
	auto b = BHPFExampel(src, 60);
	auto c = BHPFExampel(src, 160);
