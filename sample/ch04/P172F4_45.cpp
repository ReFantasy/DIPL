...
using namespace std;
Mat BLPFExampel(const Mat &src, double D0)
{
    ...
    // 生成滤波核
    auto kernel_fft = IPL::BLPF(src_fft.rows, src_fft.cols, D0, 2);
...
int main(int argc, char *argv[])
{
    std::string image_path =
        std::string(DATA_DIR) +
        "/DIP3E_Original_Images_CH04/Fig0442(a)(characters_test_pattern).tif";
    Mat src = imread(image_path, 0);

    auto b = BLPFExampel(src, 10);
    auto c = BLPFExampel(src, 30);
    auto d = BLPFExampel(src, 60);
    auto e = BLPFExampel(src, 160);
    auto f = BLPFExampel(src, 460);
