...
using namespace std;
Mat GHPFExampel(const Mat &src, double D0)
{
	...
	// 生成滤波核
	auto kernel_fft = IPL::GHPF(src_fft.rows, src_fft.cols, D0);
...
{
	std::string image_path = "../data/DIP3E_Original_Images_CH04/Fig0441(a)(characters_test_pattern).tif";
	Mat src = imread(image_path, 0);
	
	auto a = GHPFExampel(src, 30);
	auto b = GHPFExampel(src, 60);
	auto c = GHPFExampel(src, 160);
